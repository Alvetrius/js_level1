let dz = parseInt (prompt ('Введите номер проверяемого задания от 0 до 8'))
...
switch (dz) {
    ...
    case 7 :
        alert (null >= 0)
        alert ('"null" не равен, не больше и не меньше чем "0". "null" указывает на несуществующий объект — это просто специальное значение для понятия «ничто». В то время как "0" просто указывает на конкретную величину равную 0. В примере оставлен единственный результат, дающий, при сравнении, истину. Думаю это связано с оптимизацией вычисления в самом языке.')
    break

    case 8 :
        alert ('В общем, что-то я не догоняю как делать рекурсией, сделал вот так, для примера. Надеюсь, что на занятии разберёте и объясните')
        let val = parseInt (prompt ('Введите число для возведения в степень'))
        let pow = parseInt (prompt ('Введите степень, в которую необходимо возвести число'))
        let current = val
        function power (f, j) {
            let n = 0
            while (n < j-1) {
